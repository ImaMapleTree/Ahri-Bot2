...
from os import path
import pathlib
from _epy.quicktools import JOpen, DictParser
import glob
...
class CabinetDict(dict, metaclass=CDMeta):
	...
	def __getitem__(self, key):
		if not key in self:
			if self.auto_create: self[key] = None; self.cabinet.dump(self); print(f"Created entry for missing config-key ({key})")
			else: raise KeyError
		return super().get(key)
		
	def get(self, key, default=None):
		if not key in self:
			if self.auto_create: self[key] = default; self.cabinet.dump(self); print(f"Created entry for missing config-key ({key})")
			else: return default
...
class cabinet():
	...
	def from_source(cls, source, indent="\t"):
		...
		formatting = JOpen(f"{filename}.ivf", "r+")
	...
	def dump_format(self):
		...
		for key in DP.addresses.keys():
			...
			if self.format:
				...
				if not DV: FDICT[key] = type(value).__name__
				else: FDICT[key] = DV
			else:
				if type(value) == type(""): FDICT.pop(key); continue
				FDICT[key] = type(value).__name__
		if self.format != FDICT: JOpen(f"{filename}.ivf", "w+", FDICT)
	
	def dump(self, object=None, source=None):
		if not source: source = self.source
		if not self.ranked: print("[Warning] Attempted to dump a Cabinet that hasn't been ranked yet, if this is intentional you can ignore this message.")
	...
	def rank(self, start_index=0, object=None):
		...
		def format(object, key, value):
			...
			try: int(value); isInt = True
			except: isInt = False
			if isinstance(value, dict) and isObjList: object.append(value); return
			elif isinstance(value, dict): object[key] = value; return
			if type(object) == type([]) and isInt: object.append(int(value)); return
			elif type(object) == type([]): object.append(value); return
			elif not isList and value.lower() == "true": object[key] = True; return
			elif not isList and value.lower() == "false": object[key] = False; return
			elif not isList and not isObjList and isInt: object[key] = int(value); return
			if isinstance(value, list) or isinstance(value, dict): object[key] = value; return
			if value.find("|s") != -1: value = str(value.replace("|s", "")); self.createFormat = True
			elif value.find("|f") != -1: value = float(value.replace("|f", "")); self.createFormat = True
		...
		for line in self.input_list:
			if start_index < self._furthest: start_index = self._furthest
			elif start_index > self._furthest: self._furthest = start_index
			if index < start_index: index+=1; continue;
			if self.CSCheck(line) < self._CSPACE: self._CSPACE -= 1; break
			SKEY = line.find(": ")
			OKEY = line.find("{")
			LKEY = line.find("[")
			if isObj and SKEY != -1: key = line[:SKEY].replace("\t", "").replace("\r", ""); value = line[SKEY+2:]; format(object, key, value)
			elif isObj and OKEY != -1: key = line[OKEY+1:].replace("}", "").replace("\r", ""); value = {}; format(object, key, value); self._CSPACE += 1; self.rank(index+1, object[key])
			elif isList and OKEY != -1: object.append({}); self._CSPACE += 1; self.rank(index+1, object[len(object)-1])
			elif isList and LKEY != -1: object.append([]); self._CSPACE += 1; self.rank(index+1, object[len(object)-1])
			elif isObj and LKEY != -1: key = line[LKEY+1:].replace("]", "").replace("\r", ""); value = []; format(object, key, value); self._CSPACE += 1; self.rank(index+1, object[key])
