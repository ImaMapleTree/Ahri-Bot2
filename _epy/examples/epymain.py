...
def getInfection(host, victims):
	global cdict
	global last_peeps
	if host in last_peeps: return
	if not host["infected"]: return
	color = host["color"]
	for victim in victims:
		...
		if victim["color"] != color:
			...
			victim.color(color)
			cdict[color][1] += 1
	last_peeps = list(victims)
	last_peeps.append(host)
...
class MovingThread(threading.Thread):
	def __init__(self, agents, clock, *args, **kwargs):
		...
		self.agents = agents
		self.clock = clock
		self.clock.addThread(self, self.tick)
...
def createAgents(window, n, r=4, infected=random.randint(1,8)):
	rm = RandomMove(random.randint(5,20))
	agents = []
	used_colors = {}
	for i in range(n):
...
agents, colors = createAgents(window, 5, 30)
threads = createThreads(5, 1, agents, window.getInnerClock())
...
#window.getInnerClock().setDelay(0.01)
window.getInnerClock().setFramerate(60)
window.clock.run_secondary_clock()
