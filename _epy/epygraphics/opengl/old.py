...
class Sand(PhysicsObject):
	...
	def update_function(self, ticks=0):
		...
		if self.coords != self.last_coords:
			self.last_coords = self.coords
			self.no_movement = 0
		self.no_movement += 1
		if self.no_movement > 100:
			self.movement = False
		return self
...
class Water(PhysicsObject):
	...
	def force(self, *m):
		...
		if existing_pixel:
			force_direction = self.movement*-1
			existing_pixel.force(0, force_direction)
