...
class VBO_Optimizer():	
	...
	def addType(self, data):
		...
		offset_buffer = glGenBuffers(1);
		
		pregen_offsets = np.zeros(self.mvs*2, np.float32)
			
		self.offsets.append([offset_buffer, pregen_offsets, 0])
		indexed_list = self.offsets[self.index]
	...
	def addOffsetInstance(self, index, data):
		triple_list = self.offsets[index]
		pregen_data = triple_list[1]
		index_index = triple_list[2]
		
		m2 = index_index*2
		pregen_data[m2] = data[0]
		pregen_data[m2+1] = data[1]
	...
	def updateOffsetInstance(self, MI, II, data):
		offsets = self.offsets[MI][1]
		m2 = II*2
		offsets[m2] = data[0]
		offsets[m2+1] = data[1]
